...
import { CheckCircle, Truck, Mail, Ticket, Wallet, Package, Info, Plus, ChevronLeft, MapPin, ShieldCheck } from 'lucide-react';
...
const Checkout: React.FC<CheckoutProps> = ({ cart, clearCart }) => {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'paystack' | 'cash_on_delivery' | 'transfer_on_delivery'>('paystack');
  ...
  const handleAddAddress = () => {
    if (!newAddressLine || !newAddressState || !newAddressCountry || !newAddressPhone) return;
  ...
  const handleNext = () => {
    if (currentStep === 1) {
      ...
      if (!customerName || !customerEmail || !customerPhone || !customerState) {
        alert("Please completely fill out the Contact Information (Name, Email, Phone, State).");
        return;
      }
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center bg-primary-50">
        ...
        >
          ...
          <Link to="/" className="inline-block bg-primary-950 text-white px-8 py-3 font-medium rounded hover:bg-primary-900 transition-colors tracking-widest uppercase text-xs">
            Continue Shopping
          </Link>
        </motion.div>
      </div>
            ...
            {steps.map((step, idx) => (
              <React.Fragment key={step.id}>
                <div className="flex flex-col items-center relative">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${step.id < currentStep ? 'bg-green-500 text-white' :
                    step.id === currentStep ? 'bg-primary-900 text-white' : 'bg-gray-200 text-gray-400'
                    }`}>
                    {step.id < currentStep ? <CheckCircle className="w-5 h-5" /> : step.id}
                  </div>
                  <span className={`absolute -bottom-6 text-[10px] w-24 text-center uppercase tracking-widest font-bold ${step.id <= currentStep ? 'text-primary-950' : 'text-gray-400'
                    }`}>
                    {step.label}
                  </span>
                </div>
                {idx < steps.length - 1 && (
                  <div className={`flex-grow h-[2px] mx-4 transition-colors ${step.id < currentStep ? 'bg-green-500' : 'bg-gray-200'
                    }`} />
                )}
              </React.Fragment>
            ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-white p-6 sm:p-8 rounded-sm shadow-sm border border-gray-100 border-t-4 border-t-primary-950">
                  <div className="flex justify-between items-end mb-6 border-b border-gray-100 pb-4">
                    <div>
                      <h2 className="font-serif text-2xl font-bold flex items-center gap-2 text-primary-950">
                        <Package className="w-6 h-6 text-primary-600" /> Cart Review
                      </h2>
                      <p className="text-sm text-slate-500 font-light mt-1">Please review your order before proceeding.</p>
                    </div>
                    <div className="text-right hidden sm:block">
                      <p className="text-xs uppercase tracking-widest text-slate-400 font-bold">Total Items</p>
                      <p className="font-bold text-primary-600">{cart.length} items</p>
                    </div>
                  </div>
                  ...
                  {cart.length === 0 ? (
                    ...
                    <div className="divide-y divide-gray-100">
                      {cart.map((item, i) => (
                        <div key={`${item.id}-${i}`} className="py-4 flex gap-6 items-center">
                          <div className="w-16 h-20 bg-gray-100 rounded-sm overflow-hidden flex-shrink-0">
                            <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                          </div>
                          <div className="flex-grow">
                            <h4 className="font-serif font-bold text-primary-950">{item.name}</h4>
                            <div className="flex gap-8 mt-2">
                              <div>
                                <p className="text-[10px] uppercase font-bold tracking-widest text-slate-400">Quantity</p>
                                <p className="text-sm font-medium text-slate-700">{item.quantity}</p>
                              </div>
                              <div>
                                <p className="text-[10px] uppercase font-bold tracking-widest text-slate-400">Size</p>
                                <p className="text-sm font-medium text-slate-700 uppercase">{item.selectedSize || '-'}</p>
                              </div>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className="font-bold text-primary-950">${item.price.toLocaleString()}</p>
                            <p className="text-xs text-slate-400 line-through mt-1">${(item.price * 1.2).toLocaleString()}</p>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              >
                ...
                <div className="bg-white p-6 sm:p-8 rounded-sm shadow-sm border border-gray-100 border-t-4 border-t-primary-950">
                  <h2 className="font-serif text-2xl font-bold flex items-center gap-2 text-primary-950 mb-1">
                    <Mail className="w-6 h-6 text-primary-600" /> Contact Information
                  </h2>
                  <p className="text-sm text-slate-500 mb-6 font-light">Enter your details to receive order updates.</p>
                ...
                <div className="bg-white p-6 sm:p-8 rounded-sm shadow-sm border border-gray-100 border-t-4 border-t-primary-950">
                  <h2 className="font-serif text-2xl font-bold flex items-center gap-2 text-primary-950 mb-1">
                    <Truck className="w-6 h-6 text-primary-600" /> Delivery Information
                  </h2>
                  <p className="text-sm text-slate-500 mb-6 font-light">Choose how you'd like to receive your order</p>
                  ...
                  <div className="space-y-3 mb-8">
                    ...
                    >
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${deliveryMethod === 'pickup' ? 'border-primary-600' : 'border-gray-300'}`}>
                        {deliveryMethod === 'pickup' && <div className="w-2.5 h-2.5 bg-primary-600 rounded-full" />}
                      </div>
                      <div>
                        <div className="flex items-center gap-2 text-primary-950 font-bold text-sm uppercase tracking-wider">
                          <Package className="w-4 h-4 text-primary-600" /> Pickup Station
                        </div>
                        <p className="text-xs text-slate-500 mt-1">Collect your order from one of our pickup locations</p>
                      </div>
                    </button>
                    ...
                    >
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${deliveryMethod === 'delivery' ? 'border-primary-600' : 'border-gray-300'}`}>
                        {deliveryMethod === 'delivery' && <div className="w-2.5 h-2.5 bg-primary-600 rounded-full" />}
                      </div>
                      <div>
                        <div className="flex items-center gap-2 text-primary-950 font-bold text-sm uppercase tracking-wider">
                          <Truck className="w-4 h-4 text-primary-600" /> Third Party Delivery
                        </div>
                        <p className="text-xs text-slate-500 mt-1">Direct delivery to your chosen address</p>
                      </div>
                    </button>
                  </div>
                  ...
                  {deliveryMethod === 'pickup' && (
                    <div className="animate-in fade-in slide-in-from-top-4 duration-500">
                      <h3 className="text-sm font-bold text-primary-950 uppercase tracking-widest mb-4">Select Pickup Station</h3>
                      <button
                        onClick={() => setSelectedPickup('afe-babalola')}
                      ...
                      >
                        <div className={`w-4 h-4 mt-1 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${selectedPickup === 'afe-babalola' ? 'border-primary-600' : 'border-gray-300'}`}>
                          {selectedPickup === 'afe-babalola' && <div className="w-2 h-2 bg-primary-600 rounded-full" />}
                        </div>
                        <div className="flex-grow">
                          <div className="flex justify-between items-start">
                            <span className="font-bold text-primary-950 uppercase tracking-widest text-sm text-[13px]">Afe Babalola University</span>
                            <span className="font-bold text-primary-600 text-sm">$10.00</span>
                          </div>
                  ...
                  {deliveryMethod === 'delivery' && (
                    <div className="animate-in fade-in slide-in-from-top-4 duration-500">
                      <div className="bg-blue-50 border border-blue-100 p-4 rounded-sm flex gap-3 mb-6">
                        ...
                        <div>
                          <h4 className="font-bold text-blue-900 text-sm mb-1">Third Party Delivery Service</h4>
                          <p className="text-xs text-blue-800/80 leading-relaxed font-light">SANTOS uses a leading third-party luxury dispatch service to deliver products to your location safely. The delivery fee depends on the destination.</p>
                        </div>
                      </div>

                      <div className="flex justify-between items-center mb-4">
                        <h3 className="text-sm font-bold text-primary-950 uppercase tracking-widest">Select Delivery Address</h3>
                        {!isAddingAddress && (
                          <button
                      ...
                      {addresses.length > 0 && !isAddingAddress && (
                        <div className="space-y-3 mb-6">
                          {addresses.map(addr => (
                            ...
                            >
                              <div className={`w-4 h-4 mt-1 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${selectedAddressId === addr.id ? 'border-primary-600' : 'border-gray-300'}`}>
                                {selectedAddressId === addr.id && <div className="w-2 h-2 bg-primary-600 rounded-full" />}
                              </div>
                              <div className="flex-grow">
                                <div className="flex items-center gap-2 mb-1">
                                  <span className="font-bold text-primary-950 text-sm">{addr.type}</span>
                                  {selectedAddressId === addr.id && <span className="bg-primary-100 text-primary-800 text-[10px] px-2 py-0.5 rounded font-bold uppercase">Selected</span>}
                                </div>
                                <p className="text-xs text-slate-600">{addr.address}</p>
                                <p className="text-xs text-slate-500 mt-0.5">{addr.state}, {addr.country}</p>
                      ...
                      {(isAddingAddress || addresses.length === 0) && (
                        <div className="bg-slate-50 border border-slate-200 p-5 rounded-sm">
                          <h4 className="font-bold text-sm text-primary-950 mb-4 uppercase tracking-widest">New Address Details</h4>
              ...
              >
                ...
                <div className="bg-white p-6 sm:p-8 rounded-sm shadow-sm border border-gray-100 border-t-4 border-t-primary-950">
                  <div className="flex justify-between items-end mb-6 border-b border-gray-100 pb-4">
                    <div>
                      <h2 className="font-serif text-2xl font-bold flex items-center gap-2 text-primary-950">
                        <Package className="w-6 h-6 text-primary-600" /> Order Review
                      </h2>
                      <p className="text-sm text-slate-500 font-light mt-1">Please review your order before proceeding to payment.</p>
                    </div>
                    <div className="text-right hidden sm:block">
                      <p className="text-xs uppercase tracking-widest text-slate-400 font-bold">Order Summary</p>
                      <p className="font-bold text-primary-600">{cart.length} items</p>
                    </div>
                  </div>
                ...
                <div className="bg-white p-6 sm:p-8 rounded-sm shadow-sm border border-gray-100 border-t-4 border-t-primary-950">
                  ...
                  <p className="text-sm text-slate-500 mb-6 font-light">Choose your preferred payment method.</p>
                  ...
                  <div className="space-y-4">
                    ...
                    <button
                      ...
                      className={`w-full p-4 border rounded-sm flex flex-col items-start gap-2 transition-all ${paymentMethod === 'cash_on_delivery' ? 'border-primary-600 bg-primary-50/50' : 'border-gray-200 hover:border-primary-300'}`}
                    >
                      <div className="flex justify-between w-full items-center">
                        <span className="font-bold text-sm uppercase tracking-wider text-primary-950">Pay with Cash on Delivery</span>
                        <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${paymentMethod === 'cash_on_delivery' ? 'border-primary-600' : 'border-gray-300'}`}>
                          {paymentMethod === 'cash_on_delivery' && <div className="w-2 h-2 bg-primary-600 rounded-full" />}
                        </div>
                      </div>
                      <p className="text-xs text-slate-500 text-left">Pay physically when the order is delivered.</p>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-6 sm:p-8 rounded-sm shadow-sm border border-gray-100 sticky top-36">
              <h3 className="font-serif text-xl font-bold mb-6 text-primary-950">Order Summary</h3>

              {/* Promo Code Input */}
              <div className="relative mb-6">
                <input type="text" placeholder="ENTER PROMO CODE" className="w-full bg-slate-50 border border-slate-200 p-4 pl-10 text-xs font-bold tracking-widest outline-none focus:border-primary-400 transition-colors uppercase" />
              ...
              <div className="space-y-4 text-sm mb-6 border-b border-gray-100 pb-6">
                ...
                <div className="flex justify-between text-slate-600">
                  <span>Delivery <span className="text-[10px] text-primary-500 ml-1 block mt-0.5">{deliveryMethod === 'pickup' ? '(Pickup)' : '(Third Party)'}</span></span>
                  <span className="font-medium text-slate-900">{shippingCost === 0 ? '-' : `$${shippingCost.toLocaleString()}`}</span>
                </div>
              ...
              <div className="flex items-start gap-3 text-xs text-slate-500 bg-blue-50/50 p-4 rounded-sm border border-blue-100/50">
                ...
                <p className="leading-relaxed">All transactions are secure and encrypted. We never store your credit card details.</p>
              </div>
